...
namespace BUS
{
    public class BUS_News
    {
        ...
        public void updateNew(string id, string title, string photo, string detail, int status_News, string sumary)
        {
        ...
        public DataTable findNews(string keyword, string active)
        {
            string sql = "select * from News where (new_ID like N'%" + keyword + "%' or title like N'%" + keyword + "%' )"; 
            if (active != "none") sql += "and status_News=" + active ;
            return da.GetTable(sql);
        }
    }
}
